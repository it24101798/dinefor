    ...
    <div className="reservation-timeline">
      {items.map((item, index) => (
        <div
          ...
          key={`${item.status}-${item.at || index}-${index}`}
        >
        ...
        bookings.slice(0, 8).map((booking) => (
          <article className="ops-booking-card" key={booking._id}>
            ...
            <div className="ops-meta-row">
              <span>{booking.seats || 0} seat(s)</span>
            ...
            <div className="ops-actions">
              ...
              {["checked_in", "dining", "confirmed"].includes(
...
function HotelReservationCenter({ headers, setMessage }) {
  ...
  const loadBookings = async () => {
    try {
      ...
      const reservationsUrl = queryString
        ? `/hotel-portal/reservations?${queryString}`
        : "/hotel-portal/reservations";

      const [bookingsRes, opsRes, calendarRes] = await Promise.all([
        api.get(reservationsUrl, { headers }),
        api.get("/hotel-portal/reservation-operations", { headers }),
        api.get("/hotel-portal/reservation-calendar", { headers }),
      ]);
      ...
      setCalendar(
        Array.isArray(calendarRes.data) ? calendarRes.data : []
      );
    } catch (error) {
    ...
    try {
      ...
      setMessage?.(
        response.data?.message ||
          "Open reservations checked for expiry."
      );
      ...
      setMessage?.(
        error.response?.data?.message ||
          "Failed to expire open bookings."
      );
    }
  };
  ...
  const exportCsv = () => {
    if (!bookings.length) {
      setMessage?.("There are no reservations to export.");
      return;
    }
    ...
    <section className="hotel-panel reservation-ops-panel">
      <div className="hotel-panel-head">
        <div>
          ...
          <h2>
            Manage QR, check-ins, calendar and guest lifecycle
          </h2>

          <p>
            Run today&apos;s buffet operations from one control
            center: check-in guests, complete bookings, export lists,
            and monitor status flow.
          </p>
        </div>
      ...
      <div className="reservation-ops-stats">
        <div>
          <span>Total</span>
          <strong>{ops?.stats?.total || 0}</strong>
        </div>

        <div>
          <span>Today</span>
          <strong>{ops?.stats?.today || 0}</strong>
        </div>

        <div>
          <span>Upcoming</span>
          <strong>{ops?.stats?.upcoming || 0}</strong>
        </div>

        <div>
          <span>Checked In</span>
          <strong>{ops?.stats?.checkedIn || 0}</strong>
        </div>

        <div>
          <span>Completed</span>
          <strong>{ops?.stats?.completed || 0}</strong>
        </div>

        <div>
          <span>Revenue</span>
          <strong>{fmtMoney(ops?.stats?.revenue || 0)}</strong>
        </div>
      </div>
      ...
      {activeTab === "operations" && (
        <div className="reservation-ops-board">
          <OperationColumn
            title="Today"
            bookings={ops?.operational?.today || []}
            onAction={updateBooking}
          />

          <OperationColumn
            title="Checked In / Dining"
            bookings={ops?.operational?.checkedIn || []}
            onAction={updateBooking}
          />

          <OperationColumn
            title="Upcoming"
            bookings={ops?.operational?.upcoming || []}
            onAction={updateBooking}
          />

          <OperationColumn
            title="Completed"
            bookings={ops?.operational?.completed || []}
            onAction={updateBooking}
          />
        </div>
      )}
        ...
        <div className="reservation-calendar-grid">
          {calendar.length === 0 ? (
            <p className="empty-state">
              No reservations in the upcoming calendar range.
            </p>
          ) : (
            calendar.map((day) => (
              <article
                className="reservation-day-card"
                key={day.dateKey}
              >
                <div className="day-card-head">
                  <strong>
                    {new Date(day.dateKey).toLocaleDateString(
                      undefined,
                      {
                        weekday: "short",
                        month: "short",
                  ...
                  <span>{day.reservations || 0} booking(s)</span>
                </div>

                <div className="day-card-metrics">
                  <span>{day.seats || 0} seats</span>
                  <span>{fmtMoney(day.revenue)}</span>
                </div>

                <div className="day-status-row">
                  {Object.entries(day.byStatus || {}).map(
                    ([status, count]) => (
                      <small key={status}>
      ...
      {activeTab === "table" && (
        <>
          <div className="hotel-filter-bar">
            <input
              type="search"
              placeholder="Search guest, buffet or code"
              value={filters.q}
              onChange={(event) =>
                setFilters({
                  ...filters,
          ...
          >
            <div className="hotel-panel-head compact">
              <div>
                ...
                <h3>{selectedBooking.bookingCode || "Reservation"}</h3>

                <p>
                  {selectedBooking.user?.name || "Guest"} •{" "}
                  {selectedBooking.buffet?.title || "Buffet"}
                </p>
              </div>
            ...
            <div className="booking-info-grid upgraded">
              <p>
                <strong>Date</strong>
                <span>
                  {fmtDate(selectedBooking.selectedDate)}
                </span>
              </p>
              ...
              <p>
                <strong>Slot</strong>
                <span>
                  {selectedBooking.selectedTimeSlot?.startTime ||
                    "-"}{" "}
                  -{" "}
                  {selectedBooking.selectedTimeSlot?.endTime ||
                    "-"}
                </span>
              </p>
              ...
              <p>
                <strong>Seats</strong>
                <span>{selectedBooking.seats || 0}</span>
              </p>

              <p>
                <strong>Total</strong>
                <span>
                  {fmtMoney(selectedBooking.totalAmount)}
                </span>
              </p>
            </div>
